...
impl CustomScalars for MyScalars {
    ...
    fn carrier(kind: Self::Kind) -> BuiltinKind {
        match kind {
            MyKind::Mood => ScalarKind::String,
            MyKind::Build => ScalarKind::Int,
        }
    }
...
impl SqlLiteralParse for Custom {
    fn parse_literal(
        ...
        Ok(widen(Postgres::parse_literal(
            sql,
            ScalarKind::from_builtin(builtin),
        )?))
    }
}
...
fn a_wire_cell_decodes_through_the_carrier_and_the_conversion() {
    let decoded =
        subql::backend::decode_cell::<Custom, _>(0, ScalarKind::Custom(MyKind::Mood), |carrier| {
            assert_eq!(carrier, ScalarKind::String, "a mood travels as text");
    ...
    let decoded =
        subql::backend::decode_cell::<Custom, _>(1, ScalarKind::Custom(MyKind::Build), |carrier| {
            assert_eq!(carrier, ScalarKind::Int, "a build travels as an integer");
            Value::Int(1234)
        });
...
fn keying_is_answered_per_custom_type() {
    ...
    assert!(
        subql::term::kind_can_key::<Custom>(ScalarKind::Int),
        "builtins keep their own rule"
    );
    assert!(!subql::term::kind_can_key::<Custom>(ScalarKind::Json));
}
