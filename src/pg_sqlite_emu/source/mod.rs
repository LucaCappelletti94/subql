...
use row_lookup::{fetch_current_row, FallbackRow};
use table_meta::build_table_meta;
...
pub(super) struct TableMeta {
    ...
    /// column.
    pub(super) pk_column_indices: Vec<usize>,
}

#[derive(Clone, Debug)]
pub(super) struct ColumnMeta {
    pub(super) name: String,
    pub(super) pg_type_oid: Oid,
...
impl PgSqliteEmuSource {
    ...
    pub fn new(mut connection: SqliteConnection, pg_ddl: &str) -> Result<Self, PgSqliteEmuError> {
        let pg_catalog = ParserDB::parse::<sqlparser::dialect::PostgreSqlDialect>(pg_ddl)
            .map_err(|e| PgSqliteEmuError::Catalog(alloc::format!("{e:?}")))?;

        let translator = Pg2Sqlite::default()
            .sql(pg_ddl)
            .map_err(|e| PgSqliteEmuError::Translate(alloc::format!("{e:?}")))?;
        let sqlite_ddl = translator
            .translate_to_sql(&Pg2SqliteOptions::default())
            .map_err(|e| PgSqliteEmuError::Translate(alloc::format!("{e:?}")))?;
        // DDL from the pg2sqlite translator: diesel cannot express CREATE TABLE
        // DDL through its query DSL, so raw sql_query is correct here.
    ...
    pub fn open_in_memory(pg_ddl: &str) -> Result<Self, PgSqliteEmuError> {
        ...
        let conn = SqliteConnection::establish(":memory:")
            .map_err(|e| PgSqliteEmuError::Catalog(alloc::format!("open in-memory sqlite: {e}")))?;
    ...
    pub fn execute_sql(&mut self, sql: &str) -> Result<usize, PgSqliteEmuError> {
        // General-purpose "run any SQL" convenience for tests and doctests:
        // the caller provides arbitrary SQL, so the typed DSL cannot be used.
    ...
    pub fn inject_truncate(&mut self, table_id: TableId) -> Result<(), PgSqliteEmuError> {
        let table_name = self
            ...
            .ok_or_else(|| PgSqliteEmuError::UnknownTable(alloc::format!("table id {table_id}")))?;
        let meta = self
            .tables
            .get(&table_name)
            .cloned()
    ...
    fn name_by_table_id(&self, table_id: TableId) -> Option<String> {
        self.pg_catalog
            // table_id is u32; usize is always >= 32 bits on all stable targets
...
impl TableMeta {
    ...
    pub(super) fn column_schemas(&self) -> Vec<ColumnSchema<'_>> {
        self.columns
            .iter()
            .map(|c| ColumnSchema {
                name: c.name.as_str(),
...
mod tests {
    use super::row_lookup::{key_predicate, DynTable};
