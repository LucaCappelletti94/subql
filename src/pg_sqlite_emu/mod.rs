...
//! re-encodes it as one `pgoutput` transaction with
//! [`pg_walstream::encode_message`], decodes it with `pg_walstream`'s
//! `PgOutputDecoder`, and surfaces each row as a [`crate::PgChangeEvent`].
//! The emitted events look identical to what a real Postgres logical
...
//!
//! let mut engine: SubscriptionEngine<PgChangeEvent, DefaultIds, _> =
