...
mod tests {
    ...
    use super::*;
    use crate::backend::{Backend, CdcEvent, Postgres};
    use crate::checkpoint::NoCheckpoint;
    use crate::types::{EventKind, TableId};

    // ------------------------------------------------------------------
    // TestEvent<B>: a hand-rolled concrete `CdcEvent` used by every VM
    // behavioural test in this module.
    //
    // The event carries one `Vec<Value<B>>` per row view (Insert / Update
    // new-row, Update old-row) and a `Vec<ColumnId>` for the PK column
    // set. Every typed accessor dispatches on the requested `RowKind` to
    // the corresponding row image and returns the matching scalar variant.
    //
    // Semantics
    // * Column index out of range for the requested view -> `Missing`.
    // * `Value::Missing` at that index -> `Presence::Missing`.
    // * `Value::Null` at that index -> `Presence::Null`.
    // * Wrong scalar variant vs. accessor -> `Presence::Missing` (a
    //   compiler bug in production; test bug in this module).
    // * `RowKind::Pk` on a `col` not in `pk_columns` -> `Presence::Missing`
    //   per the handoff design gotcha 3.
    // ------------------------------------------------------------------

    pub(super) struct TestEvent<B: Backend> {
        pub kind: EventKind,
    ...
    impl<B: Backend> TestEvent<B> {
        pub fn insert(new_row: Vec<Value<B>>) -> Self {
            Self {
