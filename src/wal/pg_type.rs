...
use crate::backend::{at_float4, DeclaredType, FloatWidth, MySql, Postgres, ScalarFamily, Value};
...
pub(super) fn text_to_pg_value_by_kind(text: &str, kind: DeclaredType) -> Value<Postgres> {
    ...
    match kind.family() {
        ...
        ScalarFamily::Int => sql_scalar_text::parse_i64(text).map_or(Value::Missing, Value::Int),
        ScalarFamily::Decimal => {
            sql_scalar_text::parse_decimal(text).map_or(Value::Missing, Value::Decimal)
        }
        ScalarFamily::String => Value::String(text.to_string()),
        ScalarFamily::Bytes => {
            sql_scalar_text::parse_pg_bytea_hex(text).map_or(Value::Missing, Value::Bytes)
        }
        ScalarFamily::Uuid => Uuid::parse_str(text).map_or(Value::Missing, Value::Uuid),
        ...
        ScalarFamily::Date => sql_scalar_text::parse_date(text).map_or(Value::Missing, Value::Date),
        ScalarFamily::Time => sql_scalar_text::parse_time(text).map_or(Value::Missing, Value::Time),
        ScalarFamily::Json => serde_json::from_str(text).map_or(Value::Missing, Value::Json),
        ScalarFamily::Jsonb => serde_json::from_str(text).map_or(Value::Missing, Value::Jsonb),
...
mod tests {
    ...
    /// tests.
    fn declared(family: ScalarFamily) -> DeclaredType {
        crate::backend::declared_type_of(
            family,
    ...
    fn float_text_decodes_at_the_declared_width() {
        ...
        let single = DeclaredType::Float(FloatWidth::Single);
        let double = DeclaredType::Float(FloatWidth::Double);
    ...
    fn kind_bool_uses_pg_text_forms_only() {
        assert_eq!(
            text_to_pg_value_by_kind("t", declared(ScalarFamily::Bool)),
            Value::Bool(true)
        );
        assert_eq!(
            text_to_pg_value_by_kind("f", declared(ScalarFamily::Bool)),
            Value::Bool(false)
        );
        // pgoutput never emits these spellings; they must not decode.
        assert_eq!(
            text_to_pg_value_by_kind("true", declared(ScalarFamily::Bool)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind("1", declared(ScalarFamily::Bool)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind("", declared(ScalarFamily::Bool)),
            Value::Missing
        );
    }
    ...
    fn kind_int_extremes_and_garbage() {
        assert_eq!(
            text_to_pg_value_by_kind("0", declared(ScalarFamily::Int)),
            Value::Int(0)
        );
        assert_eq!(
            text_to_pg_value_by_kind("-9223372036854775808", declared(ScalarFamily::Int)),
            Value::Int(i64::MIN)
        );
        assert_eq!(
            text_to_pg_value_by_kind("9223372036854775807", declared(ScalarFamily::Int)),
            Value::Int(i64::MAX)
        );
        ...
        assert_eq!(
            text_to_pg_value_by_kind("3.5", declared(ScalarFamily::Int)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind("0x10", declared(ScalarFamily::Int)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind("", declared(ScalarFamily::Int)),
            Value::Missing
        );
    }
    ...
    fn kind_float_specials() {
        assert_eq!(
            text_to_pg_value_by_kind("1.5", declared(ScalarFamily::Float)),
            Value::Float(1.5)
        );
        assert_eq!(
            text_to_pg_value_by_kind("1e10", declared(ScalarFamily::Float)),
            Value::Float(1e10)
        );
        // pg emits Infinity / -Infinity / NaN for float specials.
        assert_eq!(
            text_to_pg_value_by_kind("Infinity", declared(ScalarFamily::Float)),
        ...
        match text_to_pg_value_by_kind("NaN", declared(ScalarFamily::Float)) {
            Value::Float(f) => assert!(f.is_nan()),
            other => panic!("expected NaN float, got {other:?}"),
        }
        assert_eq!(
            text_to_pg_value_by_kind("abc", declared(ScalarFamily::Float)),
            Value::Missing
        );
    }
    ...
    fn kind_decimal_high_precision_preserved() {
        ...
        assert_eq!(
            text_to_pg_value_by_kind(s, declared(ScalarFamily::Decimal)),
        ...
        assert_eq!(
            text_to_pg_value_by_kind("nope", declared(ScalarFamily::Decimal)),
            Value::Missing
        );
    }
    ...
    fn kind_bytes_decodes_pg_hex_format() {
        ...
        assert_eq!(
            text_to_pg_value_by_kind(r"\xdeadbeef", declared(ScalarFamily::Bytes)),
            Value::Bytes(vec![0xde, 0xad, 0xbe, 0xef])
        );
        assert_eq!(
            text_to_pg_value_by_kind(r"\x", declared(ScalarFamily::Bytes)),
            Value::Bytes(vec![])
        );
        // Missing prefix / bad hex / odd nibble count all fail closed.
        assert_eq!(
            text_to_pg_value_by_kind("deadbeef", declared(ScalarFamily::Bytes)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind(r"\xzz", declared(ScalarFamily::Bytes)),
            Value::Missing
        );
        assert_eq!(
            text_to_pg_value_by_kind(r"\xabc", declared(ScalarFamily::Bytes)),
            Value::Missing
        );
    }
    ...
    fn kind_uuid() {
        ...
        assert_eq!(
            text_to_pg_value_by_kind(u, declared(ScalarFamily::Uuid)),
            Value::Uuid(Uuid::parse_str(u).unwrap())
        );
        assert_eq!(
            text_to_pg_value_by_kind("not-a-uuid", declared(ScalarFamily::Uuid)),
            Value::Missing
        );
    }
    ...
    fn kind_string_is_verbatim_including_cursed_text() {
        ...
        for s in [
            ...
            assert_eq!(
                text_to_pg_value_by_kind(s, declared(ScalarFamily::String)),
    ...
    fn kind_json_vs_jsonb_same_text_distinct_variants() {
        let j = text_to_pg_value_by_kind(r#"{"k":1}"#, declared(ScalarFamily::Json));
        let jb = text_to_pg_value_by_kind(r#"{"k":1}"#, declared(ScalarFamily::Jsonb));
        ...
        assert_ne!(j, jb);
        assert_eq!(
            text_to_pg_value_by_kind("5", declared(ScalarFamily::Json)),
            Value::Json(serde_json::json!(5))
        );
        assert_eq!(
            text_to_pg_value_by_kind("[1,2]", declared(ScalarFamily::Jsonb)),
            Value::Jsonb(serde_json::json!([1, 2]))
        );
        assert_eq!(
            text_to_pg_value_by_kind("{bad", declared(ScalarFamily::Json)),
            Value::Missing
        );
    }
    ...
    fn kind_temporal_including_offset_normalization() {
        assert_eq!(
            text_to_pg_value_by_kind("2021-02-03", declared(ScalarFamily::Date)),
            Value::Date(NaiveDate::from_ymd_opt(2021, 2, 3).unwrap())
        );
        assert_eq!(
            text_to_pg_value_by_kind("2021-02-03 04:05:06", declared(ScalarFamily::Timestamp)),
            Value::Timestamp(
        ...
        match text_to_pg_value_by_kind("04:05:06", declared(ScalarFamily::Time)) {
            Value::Time(_) => {}
        ...
        assert_eq!(
            text_to_pg_value_by_kind(
                "2021-02-03 04:05:06+02",
                declared(ScalarFamily::TimestampTz)
            ),
            Value::TimestampTz(expected)
        );
        assert_eq!(
            text_to_pg_value_by_kind("nope", declared(ScalarFamily::Date)),
            Value::Missing
        );
    }
    ...
    fn json_kind_int_accepts_number_and_string() {
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(250), declared(ScalarFamily::Int)),
            Value::Int(250)
        );
    ...
    fn json_kind_int_rejects_fractional_and_overflow() {
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(1.5), declared(ScalarFamily::Int)),
            Value::Missing
        );
        assert_eq!(
        ...
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(42.0), declared(ScalarFamily::Int)),
            Value::Int(42)
        );
    }
    ...
    fn json_kind_float_accepts_number_and_string() {
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(1.25), declared(ScalarFamily::Float)),
            Value::Float(1.25)
        );
        // Integer-valued JSON number under a Float column widens to f64.
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(250), declared(ScalarFamily::Float)),
            Value::Float(250.0)
        );
        assert_eq!(
    ...
    fn json_kind_decimal_preserves_precision() {
        ...
        let Value::Decimal(d) =
            json_value_to_pg_value_by_kind(&serde_json::json!(s), declared(ScalarFamily::Decimal))
        else {
    ...
    fn json_kind_bool_accepts_bool_number_and_string() {
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(true), declared(ScalarFamily::Bool)),
            Value::Bool(true)
        );
        ...
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!("t"), declared(ScalarFamily::Bool)),
            Value::Bool(true)
        );
    ...
    fn json_kind_string_stringifies_nonstring() {
        assert_eq!(
            json_value_to_pg_value_by_kind(
                ...
                declared(ScalarFamily::String)
            ),
            Value::String("paid".to_string())
        );
        // A number under a String kind stringifies rather than dropping.
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(42), declared(ScalarFamily::String)),
    ...
    fn json_kind_uuid_diverges_by_backend() {
        ...
        let Value::Uuid(parsed) =
            json_value_to_pg_value_by_kind(&serde_json::json!(u), declared(ScalarFamily::Uuid))
        else {
    ...
    fn json_kind_temporal_parses_pg_and_offset_forms() {
        assert!(matches!(
            json_value_to_pg_value_by_kind(
                ...
                declared(ScalarFamily::Timestamp)
            ),
            Value::Timestamp(_)
        ));
    ...
    fn json_kind_json_vs_jsonb_and_nested_vs_string() {
        ...
        assert_eq!(
            json_value_to_pg_value_by_kind(&nested, declared(ScalarFamily::Json)),
            Value::Json(serde_json::json!({"a": 1}))
        );
        assert_eq!(
            json_value_to_pg_value_by_kind(&nested, declared(ScalarFamily::Jsonb)),
    ...
    fn json_kind_shape_mismatch_collapses_to_missing() {
        ...
        assert_eq!(
            json_value_to_pg_value_by_kind(&serde_json::json!(true), declared(ScalarFamily::Int)),
            Value::Missing
        );
    ...
    fn temporal_wire_cell_maps_representative_values() {
        assert!(matches!(
            text_to_pg_value_by_kind("2026-01-01 00:00:00", declared(ScalarFamily::Timestamp)),
            Value::Timestamp(_)
        ));
        assert!(matches!(
            json_value_to_pg_value_by_kind(
        ...
        assert!(matches!(
            text_to_pg_value_by_kind("2026-01-01", declared(ScalarFamily::Date)),
            Value::Date(_)
        ));
        assert!(matches!(
            text_to_pg_value_by_kind("12:34:56.789", declared(ScalarFamily::Time)),
            Value::Time(_)
        ));
    }
    ...
    fn temporal_wire_cell_rejects_key_boundaries() {
        assert_eq!(
            text_to_pg_value_by_kind("2026-01-01 00:00:00", declared(ScalarFamily::TimestampTz)),
            Value::Missing
        );
        assert_eq!(
            json_value_to_pg_value_by_kind(
        ...
        assert_eq!(
            text_to_pg_value_by_kind("2026-01-01 00:00:00+00", declared(ScalarFamily::Timestamp)),
            Value::Missing
        );
    }
}
