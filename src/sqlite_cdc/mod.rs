//! In-process SQLite-backed [`CdcSource`](crate::CdcSource) for end-to-end
//! dispatch testing.
//!
...
//! consumer is a `cargo test` body wanting real DML, real change
//! capture, and the real subql engine without a Docker container.
//!
...
//! type maps to `BYTES`/BLOB are out of scope: mutations on
//! tables with such columns would fail inside the trigger. Either gate
//! such tables out of the catalog passed to
//! [`SqliteCdcSource::new`] or wait for BLOB-aware
//! encoding.
//!
//! INSERT, UPDATE, and DELETE on single-column and composite
