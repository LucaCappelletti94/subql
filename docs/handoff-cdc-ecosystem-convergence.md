...
- RESOLVED in `sql-traits` (`main`, `c78a18ed`). MySQL's inline `ENUM(...)` (and `SET(...)`) hit an `unimplemented!()` arm in `sql_traits::utils::normalize_sqlparser_type`, so subql could not even parse a MySQL table with an inline enum. The fix adds `DataType::Enum(..) => "ENUM"` and `DataType::Set(..) => "SET"`, dropping the member list exactly as the normalizer already drops `VARCHAR` length. subql then classifies the column as an unknown scalar and takes the `WireType::Text` fallback, so the enum flows as its label, the same path the Postgres domain and enum use. See `docs/uphill-sql-traits-normalize-enum-set.md`. The Postgres work never hit this because its enum was a `CREATE TYPE` (`DataType::Custom`), not `DataType::Enum`.
...
6. DONE and Docker-verified. Maxwell (MySQL) emission and round trip. `maxwell_patchset_builder` in `src/emit.rs` folds `sqlite_diff_rs::maxwell::Message` events through the same `WireCatalog`, and `tests/round_trip_mysql_maxwell_e2e.rs` runs the full two-phase cycle from a MySQL source through a Maxwell file-producer daemon, across a `BINARY(16)` UUID primary key plus a `BOOLEAN`, a non-key `BINARY(16)` UUID, and an inline `ENUM`. This is the MySQL angle of the same convergence, and it is what surfaced the `sql-traits` enum gap in section 7.
...
Branch `refactor/cdc-event-trait`. The convergence and outbound loop landed in commit `f4e13a5` (sections 1 through 5). The Maxwell (MySQL) emission and round trip (section 8 phase 6) sit on top, uncommitted. All three source vehicles (wal2json, pgoutput, Maxwell) are Docker-verified for insert, update, and delete across a UUID primary key plus a bool, a non-key uuid, and a custom type (Postgres domain and enum, MySQL inline enum), in both directions. The full bidirectional loop from section 1 is closed on every source vehicle.
...
- Emission (Phases 4 to 6). `src/emit.rs` holds `WireTable` and `WireCatalog` (subql `WireSchema` over `ParserDB`), `scalar_kind_to_wire_type`, and the per-source entry points: `wal2json_patchset_builder` / `wal2json_patchset` and `maxwell_patchset_builder` / `maxwell_patchset` (unconditional), and `pgoutput_patchset_builder` / `pgoutput_patchset` (behind the `pgoutput-emit` feature, which turns on `sqlite-diff-rs/pg-walstream`). All fold events through `DiffSetBuilder::<PatchsetFormat>::digest` over the same source-agnostic `WireCatalog`, differing only in the decoder registry. The wal2json and pgoutput adapters decode a native `UUID` (`WireType::Uuid`) as a 16-byte blob (`UuidBlob16Decoder`). MySQL has no UUID type, so a `BINARY(16)` UUID rides `WireType::Bytes` and the default `MySqlBinaryDecoder` already yields a 16-byte blob, so `maxwell_adapter` needs no override.
...
- `sql-traits` points at git branch `main`, with the lockfile at `c78a18ed`, which carries the `DataType::Enum` and `DataType::Set` normalization from section 7. Required for parsing MySQL inline enums.
...
Green bar last run, all passing: `cargo +1.88 fmt --all -- --check`, `cargo +1.88 clippy --all-targets --all-features -- -D warnings`, `cargo +1.88 test --lib --release --all-features` (393 tests, including 7 `emit` tests), `cargo +1.88 test --doc --all-features` (38 tests), `RUSTDOCFLAGS="-D warnings" cargo +1.88 doc --lib --all-features --no-deps`. Docker E2E verified: `round_trip_pg_wal2json_e2e`, `round_trip_pg_pgoutput_e2e`, and `round_trip_mysql_maxwell_e2e` (the full loop across all three source vehicles, insert plus update plus delete across a UUID primary key and the bool, uuid, and custom-type column classes), plus the Phase 1 to 3 convergence set `cdc_cross_db`, `cdc_mysql_e2e`, `apply_patchset_mysql_e2e`, `reexec_postgres`, `reexec_postgres_r2d2`, and `eviction_e2e`.
...
- Key files. Trait: `src/backend.rs`. Events: `src/wal/{change_event,wal2json,maxwell}.rs` (impls on ecosystem types), `src/wal/{pg_type,pg_streaming}.rs`, `src/sqlite_cdc/{parser,event,source}.rs`, `src/pg_sqlite_emu/`, `src/testing.rs` (`TestEvent`). Emission: `src/emit.rs`. Round-trip tests: `tests/round_trip_pg_{wal2json,pgoutput}_e2e.rs`, `tests/round_trip_mysql_maxwell_e2e.rs`, and the shared `tests/common/dispatch.rs` (Postgres) plus `tests/common/roundtrip.rs` (source-agnostic patchset rebuild). Consumers: `src/runtime/{engine,dispatch,indexes,partition}.rs`, `src/reexec/`, `src/compiler/vm.rs`. Apply and patchset: `src/patchset/`. Catalog: `src/catalog_helpers.rs`.
