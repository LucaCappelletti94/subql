...
**Status**: Phases 3-14 landed. All named follow-ups closed; only cosmetic cleanup remains.
**Current HEAD**: `86806a8 Phase 14: un-gate cdc_cross_db.rs with twin-engine setup (PG + MySQL parallel)`.
**Verified bar**: `cargo +1.88 clippy --lib --all-features -- -D warnings` clean. `RUSTDOCFLAGS=-D warnings cargo +1.88 doc --lib --all-features --no-deps` clean. `cargo +1.88 test --lib --release --features sqlite-cdc` = 354 passing; adding `diesel-typed-sqlite,diesel-typed-mysql` = 360 passing (one MySQL update-follow test carries an explicit `#[ignore]` FIXME, see below). `cargo +1.88 test --doc --release --features sqlite-cdc` = 30 passing. Non-Docker integration tests (`follow_insert_sqlite`, `persistence_column_kinds_regression`, `proptest_dispatch`, `proptest_eviction`, `proptest_row_set_delta`, `proptest_register_batch_parity`, `proptest_resume_cursor`, `proptest_reexec_plan`, `proptest_throttle`, `reexec_proptest`, `reexec_diesel`, `reexec_throttle`, `uuid_id_types`) all pass. Docker-gated tests (`cdc_cross_db`, `cdc_equivalence`, `cdc_mysql_e2e`, `eviction_e2e`, `follow_insert_postgres`, `follow_insert_mysql`, `pg_streaming_e2e`, `pgoutput_e2e`, `polling_smoke`, `polling_vs_push_benchmark`, `reexec_mysql`, `reexec_postgres`, `reexec_postgres_r2d2`) compile clean but are `#[ignore]`d. Remaining known issue: `diesel_api::render_tests::register_typed_update_follow_mysql_via_engine` ignored pending a fix to strip SET-clause binds from the follow SELECT (positional `?` collision under MySQL, tracked in-file).
