...
3. **`sqlite-diff-rs` bumped to 0.1.2** (crates.io, additive-only). Adds `pg_walstream_reverse` (mirror of the existing forward `pg_walstream` adapter): converts a session changeset into a sequence of `PgOutput`-shaped messages via `op_to_message`. Consumed by `pg_sqlite_emu`. Not a 0.2.0 because nothing existing changed.
4. **`pg_walstream` bumped from git branch to crates.io 0.7** with `default-features = false`.
5. **`close_deleted_pk_follows` fix.** `src/runtime/engine.rs` had a `TODO(phase-5)` sitting on the DELETE path that never wired up follow-close for the deleted PK. Added `extract_pk_value<E: CdcEvent>` helper that reads the PK columns off the event and routes them through the same close path used elsewhere. Guarded by two regression tests (positive + negative), and I confirmed the tests fail when the fix is reverted.
6. **`src/test_harnesses.rs` migrated + un-gated.** Rewrote the 1794-line fuzz-harness module against typed CDC (`Value<Postgres>`, `Instruction<Postgres>`, `TestEvent<Postgres>`, `PgSqliteEmuSource`, `PgOutputEvent`). Every harness function (`harness_parse_sql`, `harness_vm_eval`, `harness_aggregate_consistency`, `harness_snapshot_restore_roundtrip`, `harness_sqlite_pgoutput_e2e`) compiles and its embedded tests (18 total) pass. Un-gated in `src/lib.rs` behind `feature = "testing"`. All 10 fuzz binaries under `fuzz/fuzz_targets/` and both criterion benches now compile against the new harness.
7. **`benches/dispatch.rs` migrated.** Ported to `TestEvent<Postgres>` with `Value<Postgres>` payloads, `SubscriptionEngine<TestEvent<Postgres>, DefaultIds, ParserDB>`. `bench_catalog` picks up the pre-existing helper. `make_test_event` / `make_test_update_event` / `make_test_delete_event` / `event_corpus` all typed.
8. **Cargo pin state after these addenda**: root `Cargo.toml` uses `sqlite-diff-rs = { version = "0.1.2", default-features = false }` and `pg_walstream = { version = "0.7", default-features = false }`. `fuzz/Cargo.toml` mirrors the root's diesel `[patch]` block (crates.io + `github.com/diesel-rs/diesel` -> `LucaCappelletti94/diesel` branch `future`) and uses features `["testing", "pg-sqlite-emu"]`.
